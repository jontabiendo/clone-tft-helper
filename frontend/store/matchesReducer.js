...
async function fetchMatches(name) {
  ...
  const res = await fetch(process.env.NODE_ENV === "development" ? `/riot/${name}` : `https://clone-tft-helper.onrender.com/riot/${name}`, {
    method: "GET",
    headers: {
...
export const updateSummoner = createAsyncThunk(
  ...
  async (name, thunkApi) => {
    ...
    const res = await fetch(process.env.NODE_ENV === "development" ? `/riot/update/${name}` : `https://clone-tft-helper.onrender.com/riot/update/${name}`, {
      method: "GET",
      headers: {
        'Content-Type': 'application/json'
      }
